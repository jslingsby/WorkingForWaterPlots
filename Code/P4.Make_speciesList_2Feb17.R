...
# 2002 / 2008 / 2011 / 2014
#........
p02 <- read.delim("PlotSpc_2002_2Fev17.txt")
p08 <- read.delim("PlotSpc_2008_2Fev17.txt")
p11 <- read.delim("PlotSpc_2011_2Fev17.txt")
p14 <- read.delim("PlotSpc_2014_2Fev17.txt")
...
# 2002 / 2008 / 2011 / 2014
#........
sp02_Abun <- read.delim("subPlotSpc_2002_NbIndiv_2Fev17.txt")
sp08_Abun <- read.delim("subPlotSpc_2008_NbIndiv_2Fev17.txt")
sp11_Abun <- read.delim("subPlotSpc_2011_NbIndiv_2Fev17.txt")
sp14_Abun <- read.delim("subPlotSpc_2014_NbIndiv_2Fev17.txt")
...
length(lsp)  # 1308 species
lsp
lsp2 <- data.frame(spcID=lsp)
write.table(lsp2, file="All_species_list_CapeCom_2Fev17.txt", sep="\t", quote=F, row.names=F)
...
dim(SpcPlotYear) # 26265
tail(SpcPlotYear, 10)
write.table(SpcPlotYear, file="Spc_Plot_Year_2Fev17.txt", sep="\t", quote=F, row.names=F)
...
length(spcNOTinCapList)  # 597
# This is normal, it is due to the sampling design, where many species recorded 
# in the subplots were not added to the plot records (while they actually should have been)
...
# SAPIA db
sa <- read.delim("~/Dropbox/Travail_SA/3.ComEcol/CapeCommunities/Data/Raw/List_Alien_SAPIA_13July16.txt")
head(sa) # 547 3

# Cape Community db
cc <- read.delim("~/Dropbox/Travail_SA/3.ComEcol/CapeCommunities/Data/Raw/List_Alien_CapeCom_13July16.txt")
head(cc)  # 78
...
# iSPOT data
ii <- read.delim("~/Dropbox/Travail_SA/3.ComEcol/CapeCommunities/Data/Raw/List_Alien_iSPOT_13July16.txt")
head(ii)  # 388
...
head(tr)
unique(tr$Alien)
alienD <- tr[which(tr$Alien==1), 1]
length(alienD)

# Combine all Exotic species names
l.inv <- list(sa=sa, ii=ii, cc=cc, cpi=cpi, alienD=data.frame(spcID=alienD, Status="Alien", Origin="Doug"))
...
invSpc <- sort(unique(linv$spcID))  # 890
...
write.table(invSpc, file="Exotic_species_list_SA_2Fev17.txt", sep="\t", quote=F, row.names=F)


# Select the exotic species that are in our communities
# myExo <- invSpc[invSpc %in% lsp]
# Dougy <- alienD[alienD %in% lsp]
# Dougy[!Dougy %in% myExo]
# myExo[!myExo %in% Dougy]

write.table(alienD, file="Exotic_species_list_CapeCom_2Fev17.txt", sep="\t", quote=F, row.names=F)
